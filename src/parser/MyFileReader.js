	...
	*/
	...
	loadXmlItems(parentElem, tagName, descriptor, extras, addFunc) {
		...
		for (let i = 0;  i < elems.length; i++) {
			...
			if (tagName === 'shader') {
				this.loadChildElementsOfType(elems[i], obj, "uniforms", "uniform")
			}
	...
	loadEnvs(rootElement) {
		let elem = this.getAndCheck(rootElement, 'envs')
		this.loadXmlItems(elem, 'env', this.data.descriptors["env"], [["type", "env"]], this.data.addEnv)
	}
	/*
	 * Load skybox element
	 * 
	...
	loadShaders(rootElement) {
		let elem = this.getAndCheck(rootElement, 'shaders')
		this.loadXmlItems(elem, 'shader', this.data.descriptors["shader"], [["type", "shader"]], this.data.addShader)
	}

	async loadCameraControllers() {
		for (const key in this.data.cameras) {
			let camera = this.data.cameras[key]
			if (camera.controller !== null) {
				const mod  = await import(camera.controller)
				camera.controller = mod
			}
		}
	}
	...
	loadHUDs(rootElement) {
		...
		let huds =  rootElement.getElementsByTagName('huds')[0];
		let hudElements =  huds.getElementsByTagName('hud');

		for (let i=0; i < hudElements.length; i++) {
			let hudElement = hudElements[i];
			let id = this.getString(hudElement, "id");
			let hud = this.data.createEmptyHUD(id)
			this.loadChildren(hud, hudElement.children[0])			
		}
	}
	...
	async loadCameras(rootElement) {
		...
		await this.loadCameraControllers();
	...
	async loadNodes(rootElement) {
		...
		let narrowDistance = this.getFloat(graphs[0], "narrowDistance", false)
	
		if (narrowDistance !== null) {
			this.data.narrowDistance = narrowDistance
		}
		...
		for (let i=0; i < nodeElements.length; i++) {
			...
			await this.loadNode(nodeElement);
	...
	async loadNode(nodeElement) {
		...
		let body =  nodeElement.getElementsByTagName('body')
		if (body !== null && body.length > 0) {
			this.loadNodeBody(obj, body[0])
		}

		let layers = nodeElement.getElementsByTagName('layers')
		if (layers.length > 0) {
			this.loadLayers(obj, layers[0])
		}
		let src = this.getString(nodeElement, "controller")
		if (src !== 'null') {
			try {
				...
				throw new Error("unable to load controller '" + src + "' for node '" + id + "'.")
			}
		}

		let visible = this.getBoolean(nodeElement, "visible", false)
		obj.visible = (visible !== null ? visible : true)
	...
	/**
	 * Load the layers for a particular node element
	 * @param {*} obj the node object
	 * @param {*} layersElement the layers xml element
	 * @returns 
	 */
	loadLayers(obj, layersElement) {

		for(let i=0; i<layersElement.childNodes.length; i++) {
			let temp = layersElement.childNodes[i];
			if (temp.nodeType==1) {
				let layer = temp;
				if (layer.tagName == "layer") {
					let id = this.getString(layer, "id");
					obj.layers.push(id);
				}
			}
		}
	...
	loadNodeBody(obj, bodyElement) {
		let size = this.getVector3(bodyElement, "size");
		let id = this.getString(bodyElement, "id");
		let type = this.getString(bodyElement, "type");
		let collider = this.getString(bodyElement, "collider");


		obj.body = {id: id, type: type, size: size, collider: collider}
	}
		...
		for(let i=0; i < childrenElement.childNodes.length; i++) {
			...
			if (child.nodeType == 1) {
				...
				if (id == "noderef") {
					...
					try{
						let alias = this.getString(child, "alias",false);
						this.data.addAlias(id, alias)
					}
					catch (error) {
						// no alias
					}
