...
class MySceneData {
    ...
    constructor() {
        ...
        this.descriptors["shader"] = [
            { name: "id", type: "string" },
            { name: "vertref", type: "string" },
            { name: "fragref", type: "string" },
        ]
        ...
        this.descriptors["orthogonal"] = [
            ...
            { name: "top", type: "float" },
            { name: "controller", type: "string", required: false },
            { name: "activeHud", type: "string", required: false },
        ...
        this.descriptors["perspective"] = [
            ...
            { name: "location", type: "vector3" },
            { name: "target", type: "vector3" },
            { name: "follow", type: "string", required: false },
            { name: "controller", type: "string", required: false },
            { name: "activeHud", type: "string", required: false },
        ...
        */
        ...
        this.descriptors["particle"] = [
            { name: "color", type: "rgba", required: false, default: "1 1 1 1" },
            { name: "start", type: "vector3", required: false, default: "0 0 0" },
            { name: "finish", type: "vector3", required: false, default: "0 1 0" },
            { name: "velocity", type: "integer", required: false, default: 3 },
            { name: "loop", type: "string", required: false, default: "forever" },
        ]
    ...
    addEnv(env) {
        let obj = this.getEnv(env.id)
        if (obj !== null && obj !== undefined) {
            throw new Error("inconsistency: an env with id " + env.id + " already exists!");
        }
        this.envs.set(env.id, {type: env.type, value: env.value});
        this.createCustomAttributeIfNotExists(env)
        console.debug("added env " + JSON.stringify(env));
    }
    addShader(shader) {
        
        let obj = this.getShader(shader.id);
        if (obj !== null && obj !== undefined) {
            throw new Error("inconsistency: a shader with id " + shader.id + " already exists!");
        }
        this.shaders[shader.id] = shader;
        this.createCustomAttributeIfNotExists(shader)
        console.debug("added shader " + JSON.stringify(shader));
    }
    ...
    createEmptyHUD(id) {
        let hud = this.getHUD(id)
        if (hud !== null && hud !== undefined) {
            throw new Error("inconsistency: a HUD with id " + id + " already exists!");
        }
        hud = { id: id, children: [], loaded: false, type: "hud" };
        this.addHUD(hud);
        return hud;
    ...
    createEmptyNode(id) {
        ...
        obj = { id: id, transformations: [], materialIds: [], controller: null, body: null, visible: false, layers: [], children: [], loaded: false, type: "node", castShadows: false, receiveShadows: false };
        this.addNode(obj);
        return obj;
    }
    ...
    addHUD(hud) {
        let obj = this.getHUD(hud.id)
        if (obj !== null && obj !== undefined) {
            throw new Error("inconsistency: a HUD with id " + hud.id + " already exists!");
        }
        this.huds.set(hud.id, hud);
        this.createCustomAttributeIfNotExists(hud)
        console.debug("added hud " + JSON.stringify(hud));
    }
